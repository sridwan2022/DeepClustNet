...
from DeepClustNet import DeepClustNet
import os
import csv
...
def run_exp(dbs, da_s1, da_s2, expdir, ae_weights_dir, trials=5, verbose=0,
    ...
    for db in dbs:
        ...
        for i in range(trials):  # base
            ...
            # prepare model
            model = DeepClustNet(dims, n_clusters)
            model.compile(optimizer=optim.Adam(model.parameters(), lr=0.0001), loss='mse')

            # pretraining
            ae_weights = 'ae_weights.pth'
            if ae_weights_dir is None:
                model.pretrain(x, y, optimizer=optim.SGD(model.parameters(), lr=1.0, momentum=0.9), epochs=pretrain_epochs,
